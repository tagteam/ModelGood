...
##' @export
Diagnose <- function(x,event,positive,cutoff,comparison=">=",...){
    stopifnot((n <- length(x))==length(event))
    ## get reference value
    if (missing(positive)){
        if (is.factor(x)){
            positive <- levels(event)[1]
        } else{
              positive <- event[1]
          }
    }
    ...
    ue <- unique(event)
    if (length(ue)!=2){
        stop(paste("Second argument 'event' (the gold standard or true outcome) must have exactly two unique values.\nThis one has",length(ue),"values."))
    }
    Event <- 1*(event==positive)
    ## the test
    if (length(unique(x))<=2){
        Test <- 1*(x==positive)
        test.positive <- positive
          ...
          Test <- switch(comparison,
                         ...
                         ">="=I(x>=cutoff),
                         ">"=I(x>cutoff))
          test.positive <- switch(comparison,
                             "<="=paste0("x<=",cutoff),
                             "<"=paste0("x<",cutoff),
                             ">="=paste0("x>=",cutoff),
                             ">"=paste0("x>",cutoff))
    ...
    out <- list(positive=positive,
                test.positive=test.positive,
                n=n,
                table=table(test=Test, event=Event),
...
print.Diagnose <- function(x,digits=1,...){
    cat("Diagnostic properties of binary test:\n
Samples are positive if value (argument event) equals: ",x$positive,
 "\nSamples are tested positive if test (argument x)",ifelse(x$positive==x$test.positive," equals"," satisfies"),": ",x$test.positive,
        "\n\n  There are ",x$n," samples of which ",sum(x$table[,2])," are positive
  and ",sum(x$table[2,])," have a positive test.")
    fmt <- paste0("%1.", digits[[1]], "f")
    dd <- lapply(x$diag,function(d){
                     c(sprintf(fmt,100*d[[1]]),
                       "% ",
                       paste("[",paste(sprintf(fmt,100*d$conf.int),collapse=","),"]",sep=""))
                 })
    names(dd) <- names(x$diag)
    cat("\n\nThe true positive rate (Sensitivity) is ",dd$Sensitivity,",\n",sep="")
    cat("the true negative rate (Specificity) is ",dd$Specificity,",\n",sep="")
    cat("the positive predictive value is ",dd$PPV,",\n",sep="")
    cat("and the negative predictive value is ",dd$NPV,".\n
Shown are crude rates with exact binomial 95% confidence intervals.\n",sep="")
