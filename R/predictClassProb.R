...
predictClassProb.randomForest <- function(object, newdata){
  ...
  P <- stats::predict(object,newdata=newdata,type="prob")
  P
}

predictClassProb.mvr <- function(object, newdata, ncomp = 5){
	stopifnot(!missing(newdata))
	P <- stats::predict(object,ncomp=ncomp,newdata=newdata,type="response")
	P[,,1]
}
 
predictClassProb.autoElasticNet <- function(object, newdata,...){
  stopifnot(!missing(newdata))
  P <- stats::predict(object$enet, newx=newdata, type="response", s=object$Lambda)
  P[,,1]
}
